...
class Oyl {
    ...
    async inscriptionRevealTx({ receiverAddress, script, signer, commitTxId, }) {
    ...
    async sendBRC20({ fromAddress, fromPubKey, toAddress, spendPubKey, feeRate, altSpendPubKey, spendAddress, altSpendAddress, signer, token, amount, }) {
        ...
        try {
            ...
            const { commitPsbt, script, utxosUsedForFees } = await this.inscriptionCommitTx({
                content,
                spendAddress,
                spendPubKey,
                signer,
            ...
            const estimateSendPsbt = bitcoin.Psbt.fromBase64(sentRawPsbt, {
                network: this.network,
            });
            const sentFee = estimateSendPsbt.getFee();
            const { sentPsbt: sentFinalPsbt } = await this.inscriptionSendTx({
                toAddress,
                fromPubKey,
                spendPubKey,
                spendAddress,
    ...
    async inscriptionSendTx({ toAddress, fromPubKey, spendPubKey, spendAddress, altSpendAddress, altSpendPubKey, feeRate, utxoId, utxosUsedForFees, fee, }) {
        ...
        let txSize = (0, utils_1.calculateTaprootTxSize)(1, 0, 2);
        ...
        if (utxosToPayFee?.selectedUtxos.length > 1) {
            txSize = (0, utils_1.calculateTaprootTxSize)(utxosToPayFee.selectedUtxos.length, 0, 2);
    ...
    async sendOrdCollectible({ fromAddress, fromPubKey, toAddress, spendPubKey, feeRate, altSpendPubKey, spendAddress, altSpendAddress, signer, inscriptionId, }) {
        ...
        const fee = estimatePsbt.getFee();
        const { rawPsbt: finalPsbt } = await this.createOrdCollectibleTx({
            inscriptionId,
            fromAddress,
            fromPubKey,
            spendPubKey,
    ...
    async createOrdCollectibleTx({ inscriptionId, fromAddress, fromPubKey, spendPubKey, spendAddress, toAddress, altSpendAddress, altSpendPubKey, feeRate, fee, }) {
        ...
        let utxosToSend = (0, txbuilder_1.findUtxosToCoverAmount)(availableUtxos, finalFee);
        if (utxosToSend?.selectedUtxos.length > 1) {
            txSize = (0, utils_1.calculateTaprootTxSize)(utxosToSend.selectedUtxos.length, 0, 2);
            utxosToSend = (0, txbuilder_1.findUtxosToCoverAmount)(availableUtxos, finalFee);
        }
        if (!utxosToSend) {
            const unFilteredAltUtxos = await (0, utils_1.filterTaprootUtxos)({
                taprootUtxos: altSpendUtxos,
            });
            utxosToSend = (0, txbuilder_1.findUtxosToCoverAmount)(unFilteredAltUtxos, finalFee);
            if (utxosToSend?.selectedUtxos.length > 1) {
                txSize = (0, utils_1.calculateTaprootTxSize)(utxosToSend.selectedUtxos.length, 0, 2);
                utxosToSend = (0, txbuilder_1.findUtxosToCoverAmount)(unFilteredAltUtxos, finalFee);
            }
    ...
    async sendRune({ fromAddress, toAddress, spendPubKey, feeRate, altSpendPubKey, spendAddress, altSpendAddress, signer, runeId, amount, }) {
        ...
        const sendFee = estimatePsbt.getFee();
    ...
    async runeSendTx({ runeId, fromAddress, toAddress, amount, spendAddress, spendPubKey, altSpendPubKey, altSpendAddress, feeRate, fee, }) {
        ...
        const calculatedFee = txSize * feeRate < 250 ? 250 : txSize * feeRate;
        let finalFee = fee ? fee : calculatedFee;
        let utxosToPayFee = (0, txbuilder_1.findUtxosToCoverAmount)(spendableUtxos, finalFee + utils_1.inscriptionSats);
        if (utxosToPayFee?.selectedUtxos.length > 1) {
            txSize = (0, utils_1.calculateTaprootTxSize)(utxosToPayFee.selectedUtxos.length + psbt.inputCount, 0, 3);
            utxosToPayFee = (0, txbuilder_1.findUtxosToCoverAmount)(spendableUtxos, finalFee + utils_1.inscriptionSats);
        }
        if (!utxosToPayFee) {
            const unFilteredAltUtxos = await (0, utils_1.filterTaprootUtxos)({
                taprootUtxos: altSpendUtxos,
            });
            utxosToPayFee = (0, txbuilder_1.findUtxosToCoverAmount)(unFilteredAltUtxos, finalFee + utils_1.inscriptionSats);
            if (utxosToPayFee?.selectedUtxos.length > 1) {
                txSize = (0, utils_1.calculateTaprootTxSize)(utxosToPayFee.selectedUtxos.length + psbt.inputCount, 0, 3);
                utxosToPayFee = (0, txbuilder_1.findUtxosToCoverAmount)(spendableUtxos, finalFee + utils_1.inscriptionSats);
            }
        ...
        const feeAmountGathered = (0, utils_1.calculateAmountGatheredUtxo)(utxosToPayFee.selectedUtxos);
        const changeAmount = feeAmountGathered - finalFee - utils_1.inscriptionSats;
        for (let i = 0; i < utxosToPayFee.selectedUtxos.length; i++) {
            psbt.addInput({
                hash: utxosToPayFee.selectedUtxos[i].txId,
                index: utxosToPayFee.selectedUtxos[i].outputIndex,
    ...
    async mintRune({ toAddress, spendPubKey, feeRate, altSpendPubKey, spendAddress, altSpendAddress, signer, runeId, amount, }) {
        ...
        const mintFee = estimatePsbt.getFee();
        const { sendPsbt: finalPsbt } = await this.runeMintTx({
            runeId,
            toAddress,
            amount,
            spendAddress,
    ...
    async runeMintTx({ runeId, toAddress, amount, spendAddress, spendPubKey, altSpendPubKey, altSpendAddress, feeRate, fee, }) {
        ...
        let utxosToPayFee = (0, txbuilder_1.findUtxosToCoverAmount)(spendableUtxos, finalFee + utils_1.inscriptionSats);
        if (utxosToPayFee?.selectedUtxos.length > 1) {
            txSize = (0, utils_1.calculateTaprootTxSize)(utxosToPayFee.selectedUtxos.length, 0, 3);
            utxosToPayFee = (0, txbuilder_1.findUtxosToCoverAmount)(spendableUtxos, finalFee + utils_1.inscriptionSats);
        }
        if (!utxosToPayFee) {
            const unFilteredAltUtxos = await (0, utils_1.filterTaprootUtxos)({

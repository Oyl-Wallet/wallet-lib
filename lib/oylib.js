...
class Oyl {
    ...
    async inscriptionCommitTx({ content, spendAddress, spendPubKey, signer, altSpendPubKey, altSpendAddress, feeRate, }) {
        ...
        const script = (0, utils_1.createInscriptionScript)(signer.taprootKeyPair.publicKey.toString('hex'), content);
        const finalScript = { output: bitcoin.script.fromASM(script) };
    ...
    async inscriptionRevealTx({ receiverAddress, inscriberInfo, signer, commitTxId, }) {
    ...
    async sendBRC20({ fromAddress, fromPubKey, toAddress, spendPubKey, feeRate, altSpendPubKey, spendAddress, altSpendAddress, signer, token, amount, }) {
        ...
        try {
            ...
            const { commitPsbt, inscriberInfo, utxosUsedForFees } = await this.inscriptionCommitTx({
                content,
                spendAddress,
                spendPubKey,
                signer,

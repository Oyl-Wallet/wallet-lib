...
class Oyl {
    ...
    async runeSendTx({ runeId, fromAddress, toAddress, amount, spendAddress, spendPubKey, altSpendPubKey, altSpendAddress, feeRate, fee, }) {
        ...
        if (runeUtxoVoutTotal < feeForSend + (utils_1.inscriptionSats * 2) + 2000) {
            let utxosToPayFee = (0, txbuilder_1.findUtxosToCoverAmount)(spendUtxos, feeForSend + (utils_1.inscriptionSats * 2));
            if (utxosToPayFee?.selectedUtxos.length > 1) {
            ...
            feeAmountGathered = (0, utils_1.calculateAmountGatheredUtxo)(utxosToPayFee.selectedUtxos);
            for (let i = 0; i < utxosToPayFee.selectedUtxos.length; i++) {
                psbt.addInput({
                    hash: utxosToPayFee.selectedUtxos[i].txId,
                    index: utxosToPayFee.selectedUtxos[i].outputIndex,
        ...
        console.log('inscriptionSats * 2 :', utils_1.inscriptionSats * 2);
        const changeAmount = feeAmountGathered + runeUtxoVoutTotal - feeForSend - (utils_1.inscriptionSats * 2);
        psbt.addOutput({
            address: fromAddress,
            value: utils_1.inscriptionSats,
        });

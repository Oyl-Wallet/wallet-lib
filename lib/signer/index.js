    ...
    async signAllTaprootInputs({ rawPsbt, finalize, }) {
        ...
        for (let i = 0; i < unSignedPsbt.inputCount; i++) {
            const matchingPubKey = unSignedPsbt.inputHasPubkey(i, tweakedSigner.publicKey) ||
                unSignedPsbt.inputHasPubkey(i, (0, ordit_sdk_1.toXOnly)(tweakedSigner.publicKey));
            console.log(tweakedSigner.publicKey, unSignedPsbt.data.inputs[0].tapInternalKey);
            if (matchingPubKey) {
                console.log('valid');
